...
public class Rectangle implements Printer {
  ...
  /*
   * по принципу единственной ответственности определение методов
   * CalculateDiagonal, CalculateArea и CalculatePerimeter вынесены в отдельные
   * интерфейсы. По принципу открытости/закрытости мы не изменяем родительские
   * интерфейсы, но можем модифицировать методы
   */
  ...
  /*
   * согласно принципу инверсии зависимостей и принципу открытости/закрытости
   * метод printRes() вынесен в абстрактный интерфейс Printer, который
   * имплементируется классом
   */
  @Override
